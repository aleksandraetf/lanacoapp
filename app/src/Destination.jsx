...
*/
    ...
    handleSubmit(event) {
        ...
        ).then(response => { if (response.status === 202) { this.loadData(); this.cleanData(); this.toggle('showModal'); toast.success("Destination Saved", { position: toast.POSITION_TOP_RIGHT }); } else { this.setState({ message: "Destination not saved! Fields can not be empty and it is not possible to add existing Destination!" }) } });
    }
