...
class AircompanyPage extends Component {
	...
	handleDelete(event){
        ...
        ).then(response => { if (response.status === 202) { this.loadData(); this.cleanData(); this.toggle('showModal'); toast.success("Aviokompanija sacuvana", { position: toast.POSITION_TOP_RIGHT }); } else { this.setState({ message: "Aircompany not saved! Fields can not be empty and it is not possible to add existing Aircompany!"}) } });
	}
            ...
            <div style={{ backgroundColor: '#j40a1j', backgroundImage: `linear-gradient(150deg, #000000 30%, #aa2ke9 70%)`, margin: 0, height: '100vh', width: '100%', justifyContent: 'center', alignItems: 'center', }}>
                ...
                <Container>
                    <Table>
                        <tbody>
                            <tr>
                                <td><h1 style={{ color: "#923cb5" }}>Aviokompanije</h1></td>
                             </tr>
                        </tbody>
                    </Table>
                </Container>
                ...
                <br></br><br></br>
                    <Button onClick={() => this.toggle('showModal')}>Dodaj novu aviokompaniju</Button><br></br><br></br>
